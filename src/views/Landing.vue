  ...
  <template v-if="Hero.length > 0 && PopularReleases.length > 0">
    ...
    >
      <div className="cta-container">
        ...
        <form>
          <label htmlFor="query"
            >Search Our Database From Over 400,000+ Videogames!</label
          >
          <input
            type="text"
            size="70"
      ...
      <div className="hero-details">
        <h1>{{ Hero[0].name }}</h1>
        <span v-for="(el, index) in genres" v-bind:key="index">{{
          el.name
        }}</span>
      </div>
    </div>
...
export default {
  ...
  mounted() {
    ...
    const lastYear = `${this.getCurrentYear() -
      1}-${this.getCurrentMonth()}-${this.getCurrentDay()}`;
    axios(
